...
module.exports = options => {
  ...
  function validateCredentials(credentials, userInfo) {
    ...
    return options.comparePassword(credentials[passwordName], userInfo[passwordHashName]).then(isValid => {
  ...
  return credentials => {
    ...
    return options.findUser(credentials).then(userInfo => validateCredentials(credentials, userInfo));
  };
