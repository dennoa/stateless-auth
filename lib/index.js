...
function statelessAuth(options) {
  ...
  const respondWithUnauthorised = res => res.status(401).send([{ param: 'credentials', msg: 'unauthorised' }]);

  const respondWithError = res => err => {
    return (err) ? res.status(500).send(formatError(err)) : respondWithUnauthorised(res);
  };
  ...
  function secure(secureOptions) {
    ...
    return (req, res, next) => {
      ...
      if (!decoded.userInfo) { return respondWithUnauthorised(res); }
  ...
  Object.keys(opts.providers).forEach(provider => {
    ...
    handlers[provider] = (reqBody, res) => handlerFn(reqBody).then(userInfo =>
...
statelessAuth.passwordSupport = require('./password-support');
