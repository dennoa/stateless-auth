...
module.exports = options => {
  ...
  function definitionRef(name) {
    return '#/definitions' + (docOptions.basePath || '/auth') + '_' + name;
  }
  ...
  _.forEach(options.providers, (providerOptions, provider) => {
    paths['/' + provider] = providerPath(providerOptions, provider);
  });

  let definitions = swaggerDefinitions(docOptions.basePath);

  return _.merge({ paths: paths, definitions: definitions }, options.swagger.docs);
